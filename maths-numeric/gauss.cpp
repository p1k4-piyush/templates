...
void GaussianElimination(vector<vector<T>>& a, int limit, GAUSS_MODE mode = DEGREE)
{
    ...
    for (int c = 0; c < limit; c++)
    {
        ...
        for (int i = r; i < h; i++)
        {
            if (!IsZero(a[i][c]) && (id == -1 || (mode == DEGREE && deg[i] < deg[id]) ||
...
template <typename T> int Rank(vector<vector<T>> /*&*/ a)
{
    if (a.empty())
    {
        return 0;
...
template <typename T> vector<vector<T>> Inverse(vector<vector<T>> /*&*/ a)
{
    if (a.empty())
    {
        return a;
