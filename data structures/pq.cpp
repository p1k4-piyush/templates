...
template <typename T, typename F> class Queue
{
  public:
    vector<T>          pref;
    vector<pair<T, T>> suf;
    ...
    bool Empty()
    {
        return pref.empty() && suf.empty();
    }
    int Size()
    {
        return int(pref.size()) + int(suf.size());
    }
    void Clear()
    {
        pref.clear();
...
template <typename T, typename F> Queue<T, F> MakeQueue(const F& f)
{
    return Queue<T, F>(f);
}
