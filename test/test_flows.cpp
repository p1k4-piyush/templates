...
void test_dinic_fastflow() {
    ...
    dbg("Dinic S-T Min Cut", graphviz_dinic(din, cut));
    ...
    dbg("ISAP FastFlow Network", graphviz_fastflow(ff, fast_cut));
...
void test_matching_algorithms() {
    ...
    dbg("Edmonds Blossom Matching Shrink", graphviz_blossom(blossom_ug, shrink));
}
...
void test_global_cuts() {
    ...
    dbg("Stoer Wagner Global Min Cut", graphviz_mincut(st_graph, stoer));
    ...
    dbg("Gomory Hu Connected Trees", graphviz_gomory_hu(ht));
}
